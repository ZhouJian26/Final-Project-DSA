...
{
    char *entity_name;
    struct info_entity_menager *info_entity_menager_back_p;
...
void add_entity(char *str, info_entity_menager_t *hashtable[])
{
    ...
    if (temp_iem == NULL)
    {
        ...
        new_entity_info_p->info_entity_menager_back_p = new_entity_info_menager_p;
...
/*
    *** Status: In Progress **
    Cancella una entità dalla hashtable
    - str nome entità
    - hashtable è la hashtable delle entità
    - docker_ranking_head_p è l'head docker delle varie relazioni, serve per il delete delle relazioni
 */

/*
    Funzione Gestita: deferenzia ed elimina un relation menager dal docker relativo
    NOTA: si passa il docker target
*/
void delete_relation_menager_fromDocker(info_relation_menager_t *target_to_delete, docker_ranking_t *target_docker)
{
    //non aggiorna il docker

    //printf("Traking delete_relation_menager_fromDocker\n");
    ...
    //printf("Traking delete_relation_menager_fromDocker end\n");
}
/*
...
/*
    *** Status: In Progress **
    ...
    - docker_ranking_head_p : docker relativo
*/
docker_ranking_t *delete_relation(char *src_str, char *dest_str, char *rel_str, docker_ranking_t *docker_ranking_head_p)
{
    //si passa direttamente la docker giusta

    //printf("Traking delete_relation\n");
    char *all_in_str = malloc(sizeof(char) * (strlen(src_str) + 1 + strlen(dest_str)));
    ...
    //cerco la relazione nella hashtable da eliminare
    while (temp_iem != NULL)
    {
        if (strcmp(src_str, temp_iem->info_relation_p->src) == 0 && strcmp(dest_str, temp_iem->info_relation_p->dest) == 0)
        {
            ...
            info_relation_t *temp_r = temp_iem->info_relation_p;
...
*/
docker_ranking_t *sys_delete_relation(char *src_str, char *dest_str, char *rel_str, docker_ranking_t *docker_ranking_head_p)
{
    //prendo input
...
*/
docker_ranking_t *add_relation(int relation_hash_value, info_entity_t *src_ie, info_entity_t *dest_ie, char *rel_str, docker_ranking_t *docker_ranking_head_p)
{
    ...
    info_relation_menager_t *irm_to_add = malloc(sizeof(info_relation_menager_t));
    ...
    ir_to_add->info_relation_menager_p = irm_to_add;
    ...
    irm_to_add->info_relation_p = ir_to_add;
    irm_to_add->prev_p = NULL;
    target_docker = docker_search(rel_str, docker_update);
    irm_to_add->next_p = target_docker->hashtable[relation_hash_value];
    if (target_docker->hashtable[relation_hash_value] != NULL)
        (target_docker->hashtable[relation_hash_value])->prev_p = irm_to_add;
    target_docker->hashtable[relation_hash_value] = irm_to_add;
    ...
    if (src_iem != NULL)
    {
        ...
        if (dest_iem != NULL)
        {
            ...
            if (target_docker == NULL)
            {
                ...
                docker_update = add_relation(relation_hash_value, src_iem->entity_p, dest_iem->entity_p, rel_str, docker_ranking_head_p);
            }
            else
            {
                //se esiste il docker, va verificato se esiste la relazione o no
                info_relation_menager_t *temp_iem = target_docker->hashtable[relation_hash_value];
    ...
    while (input[0] != 'e')
    {
        ...
        if (input[0] != 'e')
        {
            if (input[0] != 'r')
            {
                ...
                {
                    if (input[3] == 'r')
                        docker_head = sys_call_add_relation(input, input2, input3, entity_hashtable, docker_head);
                    else
                        sys_call_add_entity(input, entity_hashtable);
                }
                else
                {
                    if (input[3] == 'e')
                        docker_head = sys_call_delete_entity(input, entity_hashtable, docker_head);
                    else
                        docker_head = sys_delete_relation(input, input2, input3, docker_head);
                }
            }
            else
            {
                docker_head = sys_call_report(docker_head);
