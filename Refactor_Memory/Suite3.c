...
docker_ranking_t *add_relation(info_entity_t *src_ie, info_entity_t *dest_ie, char *rel_str, docker_ranking_t *docker_ranking_head_p)
{
    ...
    char *rel_malloc = malloc(sizeof(char) * (strlen(rel_str) + 1));
    strcpy(rel_malloc, rel_str);
    ...
    if (dest_cr->ranking_node_p == NULL)
    {
        ...
        docker_update = sys_create_ranking_node(rn_to_add, rel_malloc, docker_ranking_head_p);
    }
    else
    ...
    ir_to_add->src = src_ie->entity_name;
    ir_to_add->dest = dest_ie->entity_name;
    ...
    while (input[0] != 'e')
    {
        ...
        if (input[0] != 'e')
        {
            if (input[0] != 'r')
            {
                ...
                {
                    if (input[3] == 'r')
                        docker_head = sys_call_add_relation(input, input2, input3, entity_hashtable, docker_head); // V 2.0.0
                    else
                        sys_call_add_entity(input, entity_hashtable); // V 2.0.0
                }
                else
                {
                    if (input[3] == 'e')
                        docker_head = sys_call_delete_entity(input, entity_hashtable, docker_head); // V 2.0.0
                    else
                        docker_head = sys_delete_relation(input, input2, input3, docker_head, entity_hashtable); // V 2.0.0
                }
            }
            else
            {
